...
const Navbar = () => {
    ...
    return (
        <>
            <nav className={`navbar navbar-expand-lg mainNav ${darkPage.indexOf(pathname) >= 0 ? 'position-fixed' : 'position-sticky'} top-0 w-100 z-3`}>
                <div className="container-fluid">
                    <Link className={`navbar-brand fw-bold ${darkPage.indexOf(pathname) >= 0 ? 'text-white' : 'text-dark'}`} to="/">Shivam Kashyap</Link>

                    <button className={`btn-reset navbar-toggler ${darkPage.indexOf(pathname) >= 0 ? 'text-white' : 'text-dark'} `} type="button" data-bs-toggle="offcanvas" data-bs-target="#mainNavOffCanvas" aria-controls="mainNavOffCanvas">
                    ...
                    <div className="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul className={`navbar-nav ms-auto mb-2 mb-lg-0 ${darkPage.indexOf(pathname) >= 0 ? 'text-white' : 'text-dark'}`}>
            ...
            <div className={`offcanvas offcanvas-start ${darkPage.indexOf(pathname) >= 0 ? 'bg-dark' : 'bg-light'}`} data-bs-scroll="true" tabIndex="-1" id="mainNavOffCanvas" aria-labelledby="mainNavOffCanvasLabel" >
                <div className="offcanvas-header border-bottom">
                    <Link to="/" className={`offcanvas-title text-decoration-none fs-4 fw-bold ${darkPage.indexOf(pathname) >= 0 ? 'text-light' : 'text-dark'}`} id="mainNavOffCanvasLabel">
                        Shivam Kashyap
                    </Link>
                    <button type="button" className={`btn-reset ${darkPage.indexOf(pathname) >= 0 ? 'text-white' : 'text-dark'}`} data-bs-dismiss="offcanvas" aria-label="Close">
                ...
                <div className="offcanvas-body">
                    ...
                    {/* social iconaddon */}
                    <div className='mt-4'>
                        <span className={`fw-bold fs-5 d-block mb-2 ${darkPage.indexOf(pathname) >= 0 ? 'text-white' : 'text-dark'}`}>Meet me on Social</span>
